...
import 'package:active_matrimonial_flutter_app/screens/happy_story/happy_stories_middleware.dart';
import 'package:flutter/material.dart';
import 'package:active_matrimonial_flutter_app/l10n/app_localizations.dart';
import 'package:flutter_staggered_grid_view/flutter_staggered_grid_view.dart';

class HappyStories extends StatefulWidget {
  const HappyStories({super.key});
...
class _HappyStoriesState extends State<HappyStories> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CommonAppBar(
        text: AppLocalizations.of(context)!.happy_story_screen_appbar_title,
      ).build(context),
      body: StoreConnector<AppState, AppState>(
        converter: (store) => store.state,
        builder:
            (_, state) =>
                state.happyStoriesState!.isFetching!
  ...
  Widget buildBody(BuildContext context, AppState state) {
    return SizedBox(
      ...
      child:
          state.happyStoriesState!.happyStoriesList!.isNotEmpty
              ? MasonryGridView.count(
                controller: scrollController,
                itemCount:
                    state.happyStoriesState!.happyStoriesList!.length + 1,
                crossAxisCount: 2,
                mainAxisSpacing: 14,
                crossAxisSpacing: 14,
                itemBuilder: (context, index) {
                  if (index <
                      state.happyStoriesState!.happyStoriesList!.length) {
                    return InkWell(
                      onTap: () {
                        NavigatorPush.push(
                          context,
                    ...
                    return Padding(
                      ...
                      child: Center(
                        child:
                            state.happyStoriesState!.hasMore!
