...
class PP_UserContactDetails extends StatelessWidget {
  ...
  Future<void> confirm_contact_view({UserContactViewModel? vm, id}) {
    return OneContext().showDialog<void>(
      builder: (context) {
        return AlertDialog(
          title: Text('Confirm Contact View', style: Styles.bold_arsenic_14),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(
          ...
          actions: <Widget>[
            TextButton(
              ...
              child: Container(
                ...
                child: Text('Close', style: TextStyle(color: MyTheme.arsenic)),
              ),
              onPressed: () {
...
class UserContactViewModel {
  ...
  static fromStore(Store<AppState> store) {
    return UserContactViewModel(
      ...
      phone:
          store.state.publicProfileState!.viewContactCheck
